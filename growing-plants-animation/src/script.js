...
///displays skins with glassmorphism effect
function renderSkins() {
  for(var i=0; i<skins.length; i++) {
    var s = skins[i];
    
    // Create glassmorphism effect for plant stalks
    ctx.beginPath();
...
function Segment( plant, parentSegment, basePoint1, basePoint2 ) {
  ...
  //skins
  this.skins = [];
  this.skins.push( addSk( [ this.ptE1.id, this.ptE2.id, this.ptB2.id, this.ptB1.id ], "rgba(255, 255, 255, 0.15)" ) );
}
...
///renders leaf with flat glassmorphism style
function renderLeaf( plant, leafSpan ) {
  ...
  var ah = 0.25;  // arc height (reduced for flatter look)
  
  // Create glassmorphism leaf shape
  ctx.lineWidth = 1.5;
  ctx.lineJoin = "round";
  ctx.lineCap = "round";
  
  // Leaf top with glassmorphism
  ...
  // Leaf bottom with glassmorphism
  ccpx = mpx + ( p1y - p2y ) * ah;
  ccpy = mpy + ( p2x - p1x ) * ah;
  ctx.beginPath();
